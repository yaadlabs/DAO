...
unitSpec =
  [ testCase "hasSingleToken: Empty Value, should fail" $
      assertBool "Should be False" $ hasSingleToken testValue1 adaSymbol adaToken == False
  , testCase "hasSingletoken: One Symbol, more than 1 of token, should fail" $
      assertBool "Should be False" $ hasSingleToken testValue3 symbol1 tokenName1 == False
  , testCase "hasSingleToken: Correct CurrencySymbol wrong TokenName, should fail" $
      assertBool "Should Fail at traceError" $ hasSingleToken testValue4 symbol1 tokenName2 == False
  , testCase "hasSingletoken: One Symbol, should succeed" $
      assertBool "Should be True" $ hasSingleToken testValue2 symbol1 tokenName1
  , testCase "hasSingletoken: two Symbols, one valid, should succeed" $
      assertBool "Should be True" $ hasSingleToken testValue4 symbol1 tokenName1
  ]
